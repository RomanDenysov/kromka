...
import { type CheckoutFormData, checkoutFormSchema } from '@/features/checkout/schema'
import {
...
export function useCheckoutForm({
  ...
  const initialValues = useMemo(() => {
    ...
    const customerName = user?.name ?? lastOrderPrefill?.customerInfo?.name ?? ''
    const customerEmail = user?.email ?? lastOrderPrefill?.customerInfo?.email ?? ''
    const customerPhone = user?.phone ?? lastOrderPrefill?.customerInfo?.phone ?? ''
  ...
  const selectedStoreInForm = storeOptions.find((s) => s.id === storeId) ?? null
  ...
  const hasNoAvailableDates = useMemo(() => {
    ...
    return (
      !firstAvailableDate || (restrictedPickupDates !== null && restrictedPickupDates.size === 0)
    )
  ...
  useEffect(() => {
    ...
    const firstDate = getFirstAvailableDateWithRestrictions(schedule, restrictedPickupDates)
  ...
  const onSubmit = async (value: CheckoutFormData) => {
    ...
    try {
      ...
      const result = await createB2COrder({
        ...
        paymentMethod: value.paymentMethod as Exclude<typeof value.paymentMethod, 'invoice'>,
        customerInfo,
        expectedTotalCents,
      })
