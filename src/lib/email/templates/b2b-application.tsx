import { Img, Link, render, Section, Text } from "react-email";
...
export function B2BApplicationEmail({
  ...
  return (
    <EmailLayout noFooter preview={`Nová B2B žiadosť od ${companyName}`}>
      ...
      <Section className={`${EMAIL_CARD_CLASS} mt-6 space-y-3`}>
        <Text className={EMAIL_SUBTITLE_CLASS}>Informácie o spoločnosti</Text>
        ...
        <Text className={EMAIL_PARAGRAPH_CLASS}>
          <strong className="font-semibold text-gray-900">IČO:</strong> {ico}
        </Text>
        {dic && (
          <Text className={EMAIL_PARAGRAPH_CLASS}>
            <strong className="font-semibold text-gray-900">DIČ:</strong> {dic}
          </Text>
        )}
        {icDph && (
          <Text className={EMAIL_PARAGRAPH_CLASS}>
            <strong className="font-semibold text-gray-900">IČ DPH:</strong>{" "}
            {icDph}
          </Text>
        )}
      </Section>
      ...
      <Section className="mt-8 border-gray-200 border-t pt-6 text-center">
        <Text className={EMAIL_MUTED_TEXT_CLASS}>
          Táto žiadosť bola odoslaná cez B2B registračný formulár na webovej
          stránke.
        </Text>
